...
/*
 <url:https://beta.atcoder.jp/contests/abc110/tasks/abc110_b>
 問題文============================================================
 =================================================================
 解説=============================================================
 max(X,max(x)) < min(Y,min(y)) であればok
 ================================================================
 */
fn main(){
    input!(N:i32,M:usize,X:i32,Y:i32,x:[i32;N],y:[i32;M]);
    let x:Vec<i32> = x;
    let y:Vec<i32> = y;
    let &maxx = x.iter().max().unwrap();
    let &miny = y.iter().min().unwrap();

    let L = max(maxx,X);
    let R = min(miny,Y);
    println!("{}",if L < R {"No War"}else{"War"});
}
