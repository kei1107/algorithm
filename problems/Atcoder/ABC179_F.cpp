...
struct Monoid {
    ...
    static Type Nid() { return INF; /* 遅延評価時の初期値 */}
    static Type Qid() { return INF; /* 範囲外クエリ処理時の値 */}
...
Type solve(Type res = Type()){
    int N,Q; cin >> N >> Q;
    LazySegmentTree<Monoid> LSTr(N),LSTc(N);
    ...
    LSTr.set(N-1,0);
    LSTc.set(N-1,0);
    LSTr.build(); LSTc.build();

    res = (ll)(N-2)*(N-2);
    // cout << "------------------" << endl;
    // cout << res << endl;
    // for(int i = 0; i < N;i++) cout << LSTr[i] << " "; cout << endl;
    // for(int i = 0; i < N;i++) cout << LSTc[i] << " "; cout << endl;
    // cout << "------------------" << endl;
    while(Q--){
        int com; int x;
        cin >> com >> x;
        ...
        if(com == 1){
            res -= LSTc[x-1]-1;
            LSTr.update(0,LSTc[x-1],x-1);
            LSTc.update(x-1,x,0);
        }else{
            res -= LSTr[x-1]-1;
            LSTc.update(0,LSTr[x-1],x-1);
            LSTr.update(x-1,x,0);
        }
        // cout << "------------------" << endl;
        // cout << res << endl;
