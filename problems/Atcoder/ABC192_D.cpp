...
Type solve(Type res = Type()){
    ...
    ll lv = *max_element(X.begin(),X.end()) - '0';
    ll l = lv;
    ll r = LINF+1;
    ...
    auto ok = [X,M](ll m){
        ...
        for(int i = len-1; i >= 0; i--){
            ...
            if(__builtin_mul_overflow(c,base,&c)) return false;
            if(__builtin_add_overflow(sum,c,&sum)) return false;
            if(sum > M) return false;

            if(i!=0){
                if(__builtin_mul_overflow(base,m,&base)) return false;
            }
        }
