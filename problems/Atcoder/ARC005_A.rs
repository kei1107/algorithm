...
/*
 <url:https://beta.atcoder.jp/contests/arc005/tasks/arc005_1>
 問題文============================================================
 =================================================================
 解説=============================================================
 count
...
fn main(){
    ...
    let N:usize = N;
    let mut w:Vec<String> = w;
    w[N-1].pop().unwrap();
    let takahashikuns = vec!["TAKAHASHIKUN","Takahashikun","takahashikun"];
    let mut mp = HashMap::new();
    for taka in takahashikuns.iter(){
        let t = taka.clone();
        mp.entry(t.to_string()).or_insert(0);
    }
    for s in w{
        let count = mp.entry(s).or_insert(0);
        *count += 1;
    }
    let sum:i32 = takahashikuns.into_iter().map(|s| mp[s]).sum();
    println!("{}",sum);
}