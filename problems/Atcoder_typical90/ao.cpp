...
// 面積
ll area_ll(const Polygon &p) {
    ll res = 0;
    int n = (int)p.size();
    for (int j = 0;j < n;j++){
        ll px = p[j].real(), py = p[j].imag();
        ll qx = p[(j+1)%n].real(), qy = p[(j+1)%n].imag();
        res += px * qy - py * qx;
    }
    return res/2;
}
...
Type solve(Type res = Type()){
    ...
    auto ps_hull = convex_hull(ps);

    ll S = area_ll(ps_hull);

    ll B = 0;
    for(int i = 0; i < (int)ps_hull.size(); i++){
        auto diff_p = ps_hull[(i+1)%ps_hull.size()] - ps_hull[i];

        ll dx = abs(diff_p.real()), dy = abs(diff_p.imag());
