...
Type solve(Type res = Type()){
    ...
    for(int i = 0; i < (1<<n1); i++){
        ...
        hsum[cnt].emplace_back(sum);
    }
    for(int i = 0; i <= N; i++) sort(hsum[i].begin(),hsum[i].end());
    ...
    for(int i = 0; i < (1<<n2); i++){
        ...
        res += distance(hsum[K-cnt].begin(),upper_bound(hsum[K-cnt].begin(),hsum[K-cnt].end(),P-sum));
    }
    return res;
}
int main(void) {
