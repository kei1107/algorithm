...
Type solve(Type res = Type()){
    ...
    for(int j = 0; j < N;j++){
        ...
        for(int i = 0; i < N;i++){
            if(j == i) continue;
            double theta = arg(Ps[i]-Ps[j]) * 180.0 / pi;
            thetas.emplace_back(theta);
            thetas.emplace_back(theta + 360);
        }
        sort(thetas.begin(),thetas.end());
        // cout << thetas << endl;

        for(int i = 0; i < (int)thetas.size(); i++){
            auto it = upper_bound(thetas.begin(),thetas.end(),thetas[i]+180);
            it--;

            res = max(res,*it - thetas[i]);
        }
    }
    return res;
}
int main(void) {
    ...
    //solve<ll>(0);
    cout << fixed << setprecision(12) << solve<double>() << endl;
    return 0;
}