...
Type solve(Type res = Type()){
    ...
    while(Q--){
        ...
        double ny = - L/2 * sin(theta);
        double nz = L/2 - L/2*cos(theta);

        double dxy = sqrt(X*X+abs(Y-ny)*abs(Y-ny));
        double dz = nz;

        // cout << ny << " " << nz << " | " << dxy << " " << dz << endl;
        cout << fixed << setprecision(12) << atan2(dz,dxy) * 180 / pi << endl;
