...
class MysticAndCandies {
    ...
    int minBoxes(int C, int X, vector <int> low, vector <int> high) {
        ...
        //int suml = accumulate(low.begin(),low.end(),0);
        int sumh = accumulate(high.begin(),high.end(),0);
        
        vector<pii> lh(n);
        for(int i = 0; i < n;i++) lh[i] = {low[i],high[i]};
        sort(lh.begin(),lh.end(),[](const pii& l,const pii& r){
