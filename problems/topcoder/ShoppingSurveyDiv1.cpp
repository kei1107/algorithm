...
class ShoppingSurveyDiv1 {
    ...
    int minValue(int N, int K, vector <int> s) {
        ...
        int cust1[501] = {0},cust2[501] = {0};
        for(int l = 0; l <= N;l++){
            fill(cust1,cust1+501,0);
            fill(cust2,cust2+501,0);
            
            int r = N-l;
            int rnow = 0;
            for(auto data:s){
                for(int i = 0 ; i < min(l,data);i++) cust1[i]++;
                for(int i = min(l,data); i < data;i++){
                    cust2[rnow]++;
                    if(r) (rnow+=1)%=r;
                }
            }
//            for(int i = 0 ; i < l;i++) cout << cust1[i] << " "; cout << endl;
//            for(int i = 0 ; i < r;i++) cout << cust2[i] << " "; cout << endl;
            
            int T = 0;
            for(int i = 0; i < l;i++) if(cust1[i] >= K) T++;
            for(int i = 0; i < r;i++) if(cust2[i] >= K) T++;
            res = min(res,T);
        }
        return res;
    }
