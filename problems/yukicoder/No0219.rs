...
/*
 <url:https://yukicoder.me/problems/no/219>
 問題文============================================================
 =================================================================
 解説=============================================================
 BlogA の整数部分が Zに対応し、
 10^(BlogAの少数部分)がX.Yに対応する
 ================================================================
 */
fn main(){
    input!(N:usize,Query:[(f64,f64);N]);
    let Query:Vec<(f64,f64)> = Query;
    for (A,B) in Query{
        let Z = (B* A.log10()).floor();
        let XY =  10.0f64.powf(B*A.log10()-Z);
        let X = XY.floor();
        let Y = ((XY-X)*10.0).floor();
        println!("{} {} {}",X,Y,Z);
    }
}