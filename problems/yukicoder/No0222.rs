...
/*
 <url:https://yukicoder.me/problems/no/222>
 問題文============================================================
 =================================================================
 解説=============================================================
 頑張る
...
fn main(){
    ...
    for i in 1..S.len(){
        if S[i]=='+' || S[i] =='-'{
            let plus = S[i]=='-';
            let Y:Vec<_> = S.drain(i+1..).collect();
            let X:Vec<_> = S.drain(..i).collect();

            let X:String = X.into_iter().collect();
            let Y:String = Y.into_iter().collect();

            let x:i32 = X.parse().unwrap();
            let y:i32 = Y.parse().unwrap();
            println!("{}",if plus {x+y}else{x-y});
            break;
        }
    }
}