...
/*
 <url:https://yukicoder.me/problems/no/250>
 問題文============================================================
 =================================================================
 解説=============================================================
 dpで先に答えを計算しておく
 ...
 */
fn main(){
    const INF:u64 = 1_000_000_000_000_005;
    let mut dp = vec![vec![0u64;1501];1501];
    dp[0][0] = 1;
    for i in 1..1501{
        dp[i][0] = 1;
        for j in 1..1501{
            dp[i][j] = min(INF,dp[i][j-1]+dp[i-1][j]);
        }
    }
    input!(Q:usize,Query:[(usize,usize,u64);Q]);
    for q in 0..Q{
        let (D,X,T):(usize,usize,u64) = Query[q];
        let S = dp[D][X];
        println!("{}",if S<=T{"AC"}else{"ZETUBOU"});
    }
}